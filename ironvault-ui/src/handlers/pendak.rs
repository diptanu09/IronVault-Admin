...
pub fn register(app: &AppWindow, ctx: SharedContext) {
    ...
    // --- FIND OUTWARD DAK CASE ---
    {
        ...
        if let Some(ui) = ui_weak.upgrade() { ui.set_dak_find_busy(true); }

        tokio::spawn(async move {
            let result = ctx.oracle.pendak_select_outward_case_full(&target).await;
            slint::invoke_from_event_loop({
                let ui_weak = ui_weak.clone();
                move || { if let Some(ui) = ui_weak.upgrade() { ui.set_dak_find_busy(false); } }
            }).ok();
    ...
    // --- UPDATE OUTWARD DAK CASE ---
    {
        ...
        ui.set_dak_update_busy(true);

        tokio::spawn(async move {
            let result = ctx.oracle.pendak_update_outward_case(&app_num, &section, &subject).await;
            slint::invoke_from_event_loop({
                let ui_weak = ui_weak.clone();
                move || { if let Some(ui) = ui_weak.upgrade() { ui.set_dak_update_busy(false); } }
            }).ok();
    ...
    // --- LINK CORRESPONDENCE (LETTER) ---
    {
        ...
        ui.set_dak_letter_busy(true);

        tokio::spawn(async move {
            let result = ctx.oracle.pendak_insert_outward_case(letter_payload).await;
            slint::invoke_from_event_loop({
                let ui_weak = ui_weak.clone();
                move || { if let Some(ui) = ui_weak.upgrade() { ui.set_dak_letter_busy(false); } }
            }).ok();
